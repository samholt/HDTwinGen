...
class TestGeneratedEnvEnvironment(Environment):
    ...
    def reset(self, config={}, logger=None):
        ...
        if self.env_name == 'covid-gcp':
            ...
            self.train_data, self.val_data, self.test_data, self.description = load_data(num_patients=self.config.run.trajectories, seed=self.seed, env_name=self.env_name)
            ...
            self.train_data, self.val_data, self.test_data, self.description = load_data(seed=self.seed, env_name=self.env_name)
            self.executor =  Executor(variables={'train_data': self.train_data,
                                                'val_data': self.val_data,
                                                'test_data': self.test_data,
                                        },
            ...
            self.train_data, self.val_data, self.test_data, self.description = load_data(num_trajectories=self.config.run.trajectories, name=self.env_name, seed=self.seed, force_recache=self.config.setup.force_recache, load_from_cache=self.config.setup.load_from_cache, config=config, logger=logger)
            self.executor =  Executor(variables={'train_data': self.train_data,
                                                'val_data': self.val_data,
                                                'test_data': self.test_data,
                                        },
    ...
    def evaluate_simulator_code_on_test_dataset(self, StateDifferential, config={}, logger=None):
        if self.env_name == 'Cancer':
            ...
            return gt_env.evaluate_simulator_code_on_dataset(user_code_module.d_state__dt, self.test_data, config=config, logger=logger)
        elif 'COVID' in self.env_name:
            from libs.covasim.env import CovidEnv
            gt_env = CovidEnv()
